...
class JapanMarkerTest < Minitest::Test
  ...
  def test_no_dojo_appears_twice
    # UUID 経路と CSV 救済の二重掲載を防ぐ。Clubs 側には同名クラブの
    # 二重登録が実在するため（流山・古河）、名前で数えて確かめる
  ...
  def test_every_marker_records_how_it_matched
    # CSV 救済が想定外に増えていないかを、生成物から追えるようにしておく
    unknown = jp_markers.reject { |d| %w[uuid csv].include?(d['matched_by']) }
    assert_empty unknown.map { |d| d['name_japan'] },
      'matched_by が uuid / csv のどちらでもないマーカーがあります'
  end
end
